...
static void create_status_section(lv_obj_t* parent)
{
    ...
    lv_obj_set_size(status_cont, LV_PCT(100), 50);
    ...
    lv_label_set_text(title_label, LV_SYMBOL_GPS " QMC5883L Magnetometer");
...
static void create_compass_section(lv_obj_t* parent)
{
    ...
    lv_obj_set_size(compass_cont, LV_PCT(100), 180);
    ...
    lv_obj_set_size(heading_cont, LV_PCT(90), 50);
...
static void create_data_section(lv_obj_t* parent)
{
    ...
    lv_label_set_text(values_title, LV_SYMBOL_CHARGE " Magnetic Field");
    lv_obj_set_style_text_font(values_title, &lv_font_montserrat_12, 0);
    ...
    lv_label_set_text(chart_title, LV_SYMBOL_AUDIO " Real-time Data");
...
static void update_mag_chart(const qmc5883l_service_data_t* data)
{
    ...
    // 限制范围
    x_val = (x_val < 0) ? 0 : (x_val > 100) ? 100 : x_val;
    y_val = (y_val < 0) ? 0 : (y_val > 100) ? 100 : y_val;
    z_val = (z_val < 0) ? 0 : (z_val > 100) ? 100 : z_val;
...
lv_obj_t* createPage_qmc5883l(void)
{
    ...
    if (ret == ESP_OK) {
        ...
        if (ret == ESP_OK) {
            ...
            // 更新状态显示
            if (g_status_label) {
                lv_label_set_text(g_status_label, LV_SYMBOL_WIFI " Connected  " LV_SYMBOL_REFRESH " 2.0 Hz");
